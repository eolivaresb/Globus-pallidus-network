...
///////////////// Construct connected network
    ...
    NervousSystem dgp;           // Disconnected GPe
    dgp.SetCircuitSize(N, 1);
    // Load mean Voltage trace
    gp.LoadVoltage(volt_file);
    dgp.LoadVoltage(volt_file);
    ...
    for (int i=1; i<=N; i++){
        ...
        dgp.SetNeuronW(i,NeuronsProp[2*i-1]);
        dgp.SetNeuronCV(i, 0.05);
    }
    ...
    for (int i=1; i<=N; i++){
        for (int p=1; p<=7; p++){
            ...
            dgp.PrcNeuron[i].PrcParam[p] = prc_params[7*(i-1)+p];
        }
    }
    //////////////////////////////////////////////
    //////////////////////////////////////////////
        ...
        dgp.RandomizeCircuitPhase(0.0, 1.0, rs);
        //////////////////////////////////////////////
        ...
        //////////////////////////////////////////////
        gp.SetSaveSpikes(false);// Do not save spikes from parallel small world network
        for (int rep = 1; rep<=nrep; rep++){
            ////////////////////   PSTH preStim //////////////////////////
            dgp.SetSaveSpikes(true);
            for (double t = tinit; t <0-StepSize/10; t += StepSize){
                ...
                for (int i=1; i<=N; i++)  {
                    dgp.Conductance[i] = gp.Conductance_pre[i];
                    conductances[i][tindx_cond] += dgp.Conductance[i];
                }
                dgp.EulerStep(StepSize, t, rs);
                for (int i=1; i<=N; i++)  {
                    pdens[int(1+dgp.NeuronPhase(i)*pdens_bins)][(i-1)*time_bins+tindx] +=1;
                }
            }
            dgp.TimeToFirstSpike.FillContents(-1); // rest vector with time of the first spike after stim
            /////////////////////   PSTH Stimulation  /////////////////////////
            for (double t = 0; t <tend-StepSize/10; t += StepSize){
                ...
                for (int i=1; i<=N; i++)  {
                    dgp.Conductance[i] = gp.Conductance_pre[i];
                    dgp.externalinputs[i] = gsyn * (Erev - dgp.VoltEval(dgp.NeuronPhase(i)));
                    conductances[i][tindx_cond] += dgp.Conductance[i];
                }
                dgp.EulerStep(StepSize, t, rs);
                for (int i=1; i<=N; i++)  {
            ...
            //////////////////  Relaxation between PSTHs  ////////////////////////////
            tadapt = tmin + rs.UniformRandom(0, tvar);
            dgp.SetSaveSpikes(false); // do not save adaptation time spikes (way too big files otherwise)
            ...
            for (int i=1; i<=N; i++)  {
                First_spk_file.write((char*)&(dgp.TimeToFirstSpike(i)), sizeof(float));
            }
        }
        //////////////////   Save Pdens & conductances ////////////////////////////
