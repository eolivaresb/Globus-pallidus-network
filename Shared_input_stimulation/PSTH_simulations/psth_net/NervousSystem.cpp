...
void NervousSystem::SetCircuitSize(int newsize, int newmaxchemconns)
{
    ...
    ////// Time to fist spike vector
    TimeToFirstSpike.SetBounds(1,size);
    TimeToFirstSpike.FillContents(-1.0);
...
void NervousSystem::EulerStep(double stepsize, double t, RandomState &rs)
{
    ...
    float tspk;
    // Update Conductance of all neurons and save previous one for calculations.
    ...
    for (int i = 1; i <= size; i++) { // sweep over neuron 
        ...
        if (Phases[i] >=1.0){
            ...
            if (SaveSpikes){//// Store spike
                spkt.write((char*)&tspk, sizeof(float));
                spki << i << endl;
            }
            if (TimeToFirstSpike[i]==-1){TimeToFirstSpike[i]=tspk;}
            //// Reset Phase
            Phases[i] -= 1.0;
        }
